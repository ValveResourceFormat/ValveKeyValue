...
namespace ValveKeyValue
{
    ...
    static class ObjectCopier
    {
...
#if NET5_0_OR_GREATER
        [UnconditionalSuppressMessage("ReflectionAnalysis", "IL2072", Justification = "Cannot annotate IObjectMember, but analysis performed earlier in flow should be sufficient.")]
        static void CopyObject<[DynamicallyAccessedMembers(Trimming.Properties)] TObject>(KVObject kv, TObject obj, IObjectReflector reflector)
#else
...
#endif
        {
            ...
            var members = reflector.GetMembers(typeof(TObject), obj).ToDictionary(m => m.Name, m => m, StringComparer.OrdinalIgnoreCase);
...
#else
        static TValue ConvertValue<TValue>(object value, IObjectReflector reflector)
#endif
