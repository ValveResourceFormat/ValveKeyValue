...
namespace ValveKeyValue.Test.TextKV2
{
    class KV2SerializationTestCase
    {
        ...
        /// <summary>
        /// The element name is a built-in member rather than an attribute, so a child attribute
        /// literally called "name" is never written out as one.
        /// </summary>
        [TestCase(KVSerializationFormat.KeyValues2Binary)]
        [TestCase(KVSerializationFormat.KeyValues2Text)]
        public void DoesNotWriteNameAsAttribute(KVSerializationFormat format)
        {
            var serializer = KVSerializer.Create(format);
            var root = new KV2Element("TestClass", "elementName", Guid.NewGuid());
            root.Add("name", new KVObject("attributeName"));
            root.Add("health", new KVObject(100));
            ...
            Assert.Multiple(() =>
            {
                Assert.That(root2.Name, Is.EqualTo("elementName"));
                Assert.That(root2.ContainsKey("name"), Is.False);
                Assert.That((int)root2["health"], Is.EqualTo(100));
            });
        }
